...
class GratuityCalculator(AccountsController):
	def validate(self):    
		pass
		# self.total_work_days = date_diff(self.end_work_date, self.start_work_date) + 1  # +1 لتضمين يوم البداية أيضًا
	...
	def calculate_work_experience_and_amount(self):
		...
		if rule.method == "Manual":
			...
			current_work_experience = calculate_work_experience(self, self.employee, self.gratuity_rule) or 0

		self.total_work_years = current_work_experience
...
def calculate_work_experience(self, employee, gratuity_rule):
...
def calculate_employee_total_workings_days(employee, date_of_joining, relieving_date):
	...
	frappe.msgprint("------------employee_total_workings_days-------------")
	...
	frappe.msgprint("------------employee_total_workings_days2-------------")
	frappe.msgprint(str(employee_total_workings_days))
	frappe.msgprint("-------------------------")
